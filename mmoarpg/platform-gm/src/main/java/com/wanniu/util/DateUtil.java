...
public final class DateUtil {
	...
	/** 得到几天前的时间的0点 */
	public static Calendar getDateBeforeIn0(Date time, int day) {
		Calendar now = Calendar.getInstance();
		now.setTimeInMillis(time.getTime() - day * 24 * 60 * 60 * 1000);
	...
	/** 得到几天后的时间的0点 */
	public static Calendar getDateAfterIn0(Date time, int day) {
		Calendar now = Calendar.getInstance();
		now.setTimeInMillis(time.getTime() + day * 24 * 60 * 60 * 1000);
		now.set(Calendar.HOUR_OF_DAY, 0);
		now.set(Calendar.MINUTE, 0);
