...
import cn.qeng.gm.module.query.service.PetUpgradeService;
...
public class PetUpgradeController {
	@Autowired
	private PetUpgradeService petUpgradeService;

	/**
	 * 左边导航
	 */
	@RequestMapping("/manage")
	@Auth(AuthResource.QUERY_PET)
	@RecordLog(OperationType.QUERY_PET_MANAGE)
	public ModelAndView manage() {
		return new ModelAndView("/query/pet/list");
	}

	@RequestMapping("/check")
	@Auth(AuthResource.QUERY_PET)
	@RecordLog(value = OperationType.QUERY_PET_CHECK, args = { "playerId" })
	public ModelAndView list(String playerId) {
		ModelAndView view = this.manage();
		view.addObject("page", petUpgradeService.getPetUpgradeResult(playerId));
