...
import cn.qeng.gm.module.game.service.LoginNoticeService;
 ...
 */
@Controller
@RequestMapping("/game/login/notice")
public class LoginNoticeController {
	...
	@Auth(AuthResource.MANAGER_LOGIN_NOTICE)
	...
	public ModelAndView list() {
		return new ModelAndView("game/loginnotice/list");
	}
	...
	public String query() {
		return JSON.toJSONString(loginNoticeService.getNotices());
	}
	...
	@Auth(AuthResource.MANAGER_LOGIN_NOTICE)
	@RecordLog(OperationType.MANAGER_LOGIN_NOTICE_ADDUI)
	public ModelAndView addUI() {
		return new ModelAndView("game/loginnotice/index");
	}
	...
	@ResponseBody
	@Auth(AuthResource.MANAGER_LOGIN_NOTICE)
	@RecordLog(value = OperationType.MANAGER_LOGIN_NOTICE_ADD, args = { "title" })
	@RequestMapping("/add/")
	public String add(HttpServletRequest request, String title, String content) throws ParseException {
		...
		return String.valueOf(loginNoticeService.addNotice(user, title, content));
	}
	...
	@ResponseBody
	@Auth(AuthResource.MANAGER_LOGIN_NOTICE)
	@RecordLog(value = OperationType.MANAGER_LOGIN_NOTICE_EDITUI, args = { "id" })
	@RequestMapping("/editUI/{id}/")
	public ModelAndView editUI(@PathVariable("id") int id) {
		ModelAndView view = this.addUI();
		view.addObject("notice", loginNoticeService.getNotice(id));
		return view;
	}
	...
	@RequestMapping("/edit/")
	@Auth(AuthResource.MANAGER_LOGIN_NOTICE)
	@RecordLog(value = OperationType.MANAGER_LOGIN_NOTICE_EDIT, args = { "title" })
	public String edit(HttpServletRequest request, int id, String title, String content) throws ParseException {
		SessionUser user = SessionManager.getSessionUser(request);
		return String.valueOf(loginNoticeService.editNotice(user, id, title, content));
	}
	...
	@ResponseBody
	@Auth(AuthResource.MANAGER_LOGIN_NOTICE)
	@RecordLog(value = OperationType.MANAGER_LOGIN_NOTICE_ENABLE, args = { "id" })
	@RequestMapping("/enable/{id}/")
	public ModelAndView enable(@PathVariable("id") int id) {
		loginNoticeService.enableNotice(id);
		return this.list();
	}
	...
	@ResponseBody
	@Auth(AuthResource.MANAGER_LOGIN_NOTICE)
	@RecordLog(value = OperationType.MANAGER_LOGIN_NOTICE_DELETE, args = { "id" })
