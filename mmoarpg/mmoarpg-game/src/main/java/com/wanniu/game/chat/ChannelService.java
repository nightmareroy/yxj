...
public class ChannelService {
	 ...
	 * constant
	 */
	public static final int ST_INITED = 0;
	public static final int ST_DESTROYED = 1;

	/**
	 * Create and maintain channels for server local.
	 *
	 * ChannelService is created by channel component which is a default loaded
	 * component of pomelo and channel service would be accessed by
	 * `app.get('channelService')`.
	 *
	 * @class
	 * @constructor
	 */
	// public ChannelService(app, opts) {
	// opts = opts || {};
	// this.app = app;
	// this.channels = {};
	...
	// ChannelService.prototype.start(cb) {
	...
	// */
	// ChannelService.prototype.createChannel(name) {
	...
	// * @memberOf ChannelService
	// */
	// ChannelService.prototype.getChannel(name, create) {
	...
	// * @memberOf ChannelService
	// */
	// ChannelService.prototype.destroyChannel(name) {
	...
	// * @param opts
	// */
	// ChannelService.prototype.pushMessageToPlayer(route, msg, uid){
	// var remote = this.app.components.__remote__;
	//
	// var pkg = {
	// isPush:true,
	// uid:uid.uid,
	// route:route,
	// msg:msg};
	// //logger.error('pushMessageToPlayer:',uid,pkg);
	// remote.pushMessage(uid.sid,pkg);
	// }
	/// **
	...
	// * @param {Array} uids the receiver info list, [{uid: userId, sid:
	// frontendServerId}]
	...
	// return function(err) {
	// if(err) {
	// logger.error('[broadcast] fail to push message to serverId: ' + serverId + ',
	// err:' + err.stack);
	// latch.done();
	// return;
	// }
	...
	// }
	// addToStore(this.__channelService__, genKey(this.__channelService__,
	// this.name), genValue(sid, uid));
	// return res;
	// }
	// };
	//
	...
	// */
	// Channel.prototype.leave(uid, sid) {
	// if(!uid || !sid) {
	// return false;
	// }
	...
	// removeFromStore(this.__channelService__, genKey(this.__channelService__,
	// this.name), genValue(sid, uid));
	...
	// */
	// Channel.prototype.getUserAmount() {
	//
	...
	// */
	// Channel.prototype.getMember(uid) {
	...
	// */
	// Channel.prototype.pushMessage(route, msg, opts, cb) {
	// if(this.state !== ST_INITED) {
	...
	// }
	//
	// sendMessageByGroup(this.__channelService__, route, msg, this.groups, opts,
	// cb);
	// };
	//
	/// **
	...
	// * @api private
	// */
	// var sendMessageByGroup(channelService, route, msg, groups, opts, cb) {
	...
	// var failIds = [];
	//
	// //logger.debug('[%s] channelService sendMessageByGroup route: %s, msg: %j,
	// groups: %j, opts: %j', app.serverId, route, msg, groups, opts);
	// if(count === 0) {
	// // group is empty
	// utils.invokeCallback(cb);
	// return;
	...
	// if(err) {
	// logger.error('[pushMessage] fail to dispatch msg to serverId: ' + serverId +
	// ', err:' + err.stack);
	// latch.done();
	// return;
	// }
	...
	// return (function() {
	// if(sid === app.serverId) {
	// channelService.channelRemote[method](route, msg, groups[sid], opts,
	// rpcCB(sid));
	// } else {
	...
	// };
	//
	// var removeFromStore(self, key, value) {
	// if(!!self.store) {
	...
	// };
	//
	// var loadAllFromStore(self, key, cb) {
	// if(!!self.store) {
	...
	// };
	//
	// var removeAllFromStore(self, key) {
	// if(!!self.store) {
