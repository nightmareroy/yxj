...
public abstract class AnsycPublisher {
	...
	public AnsycPublisher() {
		...
		data.put("_id", uuid);
		data.put("sid", GWorld.__SERVER_ID);
		data.put("date", now.format(DateUtils.F_YYYYMMDDHHMMSS));

		// 如果需要一天一个表，添加此参数
		if (oneDay()) {
			data.put("_one_day", true);
		}
	}

	public void publishing() {
		GWorld.getInstance().ansycExec(() -> PublishCenter.publish(channel(), JSON.toJSONString(data)));
	}
}